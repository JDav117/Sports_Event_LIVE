...
export class PlayerEnrollmentService {
  ...
  async create(
    ...
    if (
      ...
      throw new BadRequestException(
        'teamId es requerido para inscripciones de tipo team o both',
      );
    }

    if (
      (enrollmentType === EnrollmentType.EVENT ||
        enrollmentType === EnrollmentType.BOTH) &&
      !eventId
    ) {
      throw new BadRequestException(
        'eventId es requerido para inscripciones de tipo event o both',
      );
    }
    ...
    if (teamId) {
      ...
      const currentEnrollments =
        await this.teamsService.getTeamEnrollmentCount(teamId);
      ...
      if (existingEnrollment) {
        throw new BadRequestException(
          'El jugador ya está inscrito en este equipo',
        );
      }
    }
    ...
    if (eventId) {
      ...
      if (event.maxAttendees) {
        const currentEnrollments =
          await this.eventsService.getEventEnrollmentCount(eventId);
      ...
      if (existingEnrollment) {
        throw new BadRequestException(
          'El jugador ya está inscrito en este evento',
        );
      }
    }
  ...
  async findAll(filters?: {
    ...
    if (filters?.playerId) {
      queryBuilder.andWhere('enrollment.playerId = :playerId', {
        playerId: filters.playerId,
      });
    }

    if (filters?.teamId) {
      queryBuilder.andWhere('enrollment.teamId = :teamId', {
        teamId: filters.teamId,
      });
    }

    if (filters?.eventId) {
      queryBuilder.andWhere('enrollment.eventId = :eventId', {
